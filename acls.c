...
static int AclSetPermCmd __P((struct acluser *, const char *, struct comm *));
static int AclSetPermWin __P((struct acluser *, struct acluser *, const char *, struct win *));
 ...
 */
struct acluser **FindUserPtr(const char *name)
{
  struct acluser **u;
 ...
 */ 
static struct aclusergroup **FindGroupPtr(struct aclusergroup **gp,
    struct acluser *u, int recursive)
{
...
static int PasswordMatches(const char *pw, const char *password)
{
  if (!*password)
    return 0;
 ...
 */
static int AclSetPermCmd(struct acluser *u, const char *mode, struct comm *cmd)
{
  int neg = 0;
  const char *m = mode;
 ...
 */
static int AclSetPermWin(struct acluser *uu, struct acluser *u, const char *mode, struct win *win)
{
  int neg = 0;
  int bit, bits;
  AclBits *bitarray;
 ...
 * uu should be NULL, except if you want to change his umask.
 */
int AclSetPerm(struct acluser *uu, struct acluser *u, const char *mode, const char *s)
{
    struct win *w;
    int i;
    char *p, ch;
