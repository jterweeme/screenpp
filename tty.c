...
static sigret_t SigAlrmDummy SIGDEFARG
{
  debug("SigAlrmDummy()\n");
  SIGRETURN;
}
...
/*ARGSUSED*/
int TtyGrabConsole(int fd, int on, const char *rc_name)
{
...
# ifdef TIOCM_RTS
      s = (char *)"!RTS "; if (mflags & TIOCM_RTS) s++;
      while (*s) *p++ = *s++;
# endif
# ifdef TIOCM_CTS
      s = (char *)"!CTS "; 
...
# ifdef TIOCM_DTR
      s = (char *)"!DTR "; if (mflags & TIOCM_DTR) s++;
      while (*s) *p++ = *s++;
# endif
# ifdef TIOCM_DSR
      s = (char *)"!DSR "; if (mflags & TIOCM_DSR) s++;
      while (*s) *p++ = *s++;
# endif
