...
void centerline(const char *str, int y)
{
    ASSERT(flayer);
    int n = strlen(str);
    ...
    int l = (flayer->l_width - 1 - n) / 2;
    LPutStr(flayer, str, n, &mchar_blank, l, y);
}

void leftline(const char *str, int y, struct mchar *rend)
{
    int l, n;
...
sigret_t (*xsignal(int sig, sigret_t (*func)(int))) (int)
{
...
void xbcopy(char *s1, char *s2, int len)
{
  if (s1 < s2 && s2 < s1 + len)
    {
      s1 += len;
...
static int xsnoff(int a, char *b, int c)
{
    return a ? (char *)&c  - (char *)&b : (char *)&b - (char *)&a;
}

int xsnprintf(char *s, int n, char *fmt, unsigned long p1, unsigned long p2,
    unsigned long p3, unsigned long p4, unsigned long p5, unsigned long p6)
{
...
int xvsnprintf(char *s, int n, char *fmt, xva_list stack)
{
    char *f, *sf = 0;
    int i, argl = 0;
    char myf[10], buf[20];
