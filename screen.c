...
static int   IsSymbol __P((char *, const char *));
...
int main(int ac, char** av)
{
    ...
    int mflag = 0;
    const char *myname = (ac == 0) ? "screen" : av[0];
    char *SockDir;
    struct stat st;
#ifdef _MODE_T			/* (jw) */
    mode_t oumask;
...
# endif
    (void)StartRc((char *)ETCSCREENRC, 0);
#endif
...
#ifdef BSDWAIT
void WindowDied(struct win *p, union wait wstat, int wstat_valid)
#else
...
static sigret_t CoreDump SIGDEFARG
{
  ...
  struct display *disp;
  char buf[80];
  char *dump_msg = (char *)" (core dumped)";
...
static int IsSymbol(char *e, const char *s)
{
  register int l;
