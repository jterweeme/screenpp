...
static struct win *WindowByName(const char *);
static int  WindowByNumber(const char *);
static int  ParseOnOff __P((struct action *, int *));
static int  ParseWinNum __P((struct action *, int *));
static int  ParseBase(struct action *, char *, int *, int, const char *);
...
char *zmodem_sendcmd;
char *zmodem_recvcmd;
static const char *zmodes[4] = {"off", "auto", "catch", "pass"};
#endif
...
static struct action *FindKtab(char *klass, int create)
{
  ...
  if (kp == 0)
      ...
      if (strlen(klass) > 80)
	{
      ...
      kp = (struct kclass *)malloc(sizeof(*kp));
      if (kp == 0)
	{
	  Msg(0, "%s", strnomem);
	  return 0;
	}
      kp->name = SaveStr(klass);
...
{
   int i;
    static const char *argss[] = {"no", "one", "two", "three", "four", "OOPS"};
    static const char *orformat[] = 
    {
    ...
    int n = comms[nr].flags & ARGS_MASK;
...
# endif /* SIG_NODEBUG */
    ...
    case RC_BREAKTYPE:
	{
	  static const char *types[] = { "TIOCSBRK", "TCSBRK", "tcsendbreak", NULL };
...
# endif
      ...
      if (*args)
	{
      ...
      if (msgok)
#ifdef COLOR
        OutputMsg(0, "Standout attributes 0x%02x  color 0x%02x", (uint8_t)mchar_so.attr, 0x99 ^ (uint8_t)mchar_so.color);
#else
        OutputMsg(0, "Standout attributes 0x%02x ", (uint8_t)mchar_so.attr);
#endif
      break;
    ...
    case RC_SU:
	  ...
	  OutputMsg(0, "%s:%s screen login", HostName, SockPath);
        ...
        s = DoSu(&D_user, args[0], args[1], (char *)"\377");
      else
      ...
      else
	{
	  if (!windows)
            {
	      wtab = (struct win **)realloc(wtab, n * sizeof(struct win *));
      ...
      else
	{
	  ...
	  if (ParseBase(act, args[1], &lifespan, 10, "decimal"))
	    break;
	  if (ParseBase(act, args[2], &tick, 10, (char *)"decimal"))
	    break;
      ...
      else if (!strcmp(args[0], "dump"))
	{
	  ...
	  else if (!LayoutDumpCanvas(&D_canvas, args[1] ? args[1] : (char *)"layout-dump"))
      ...
      while (*p)
	{
	  if (*p == delim)
	      ...
	      else
		{
		  ...
		  v = xbuf;
		  if (!strcmp(ps, "TERM"))
		    v = display ? D_termname : (char *)"unknown";
...
static struct win *WindowByName(const char *s)
{
  struct win *p;
...
static int WindowByNumber(const char *str)
{
    int i;
    const char *s;
 ...
 */
int WindowByNoN(const char *str)
{
    int i;
    struct win *p;
...
static int
ParseBase(struct action *act, char *p, int *var, int base, const char *bname)
{
  int i = 0;
  int c;
 ...
 * -a, -M, -L
 */
void DoScreen(const char *fn, char **av)
{
 ...
 */
int
CompileKeys(char *s, int sl, uint8_t *array)
{
  int i;
  uint8_t key, value;
