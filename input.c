...
void
inp_setprompt(const char *p, const char *s)
{
 ...
 */
void Input(const char *istr, int len, int mode,
    void (*finfunc) (char *buf, int len, char *priv), char *priv, int data)
#if 0
const char *istr;
int len;
int mode;
	       ...
	       ch == '\000' || ch == '\n' || ch == '\r')
	{
	  ...
	  if (inpdata->inp.len && !(inpdata->inpmode & (INP_NOECHO | INP_RAW)))
	      ...
	      for (store = inphist.prev; store; store = store->prev)
		{
	      ...
	      if (!store)
		{
		  store = (struct inpline *)malloc(sizeof(struct inpline));
