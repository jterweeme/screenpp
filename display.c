...
void AddCStr(const char *s)
{
  if (display && s && *s)
    {
      ospeed = D_dospeed;
      tputs((char *)s, 1, DoAddChar);
    }
}

void AddCStr2(const char *s, int c)
{
    if (display && s && *s)
    {
        ospeed = D_dospeed;
        tputs(tgoto((char *)s, 0, c), 1, DoAddChar);
    }
}
...
void SetAttr(int mew)
{
  int i, j, old, typ;

  if (!display || (old = D_rend.attr) == mew)
    return;
#ifdef COLORS16
  D_col16change = (old ^ mew) & (A_BFG | A_BBG);
  mew ^= D_col16change;
  ...
  if (D_SA)
    {
      ...
      SetFont(ASCII);
      ospeed = D_dospeed;
      tputs(tparm(D_SA, mew & A_SO, mew & A_US, mew & A_RV, mew & A_BL,
			mew & A_DI, mew & A_BD, 0         , 0         ,
			0), 1, DoAddChar);
      D_rend.attr = mew;
      D_atyp = 0;
# ifdef COLOR
...
void SetFont(int mew)
{
  ...
  D_rend.font = mew;
#ifdef ENCODINGS
  if (D_encoding && CanEncodeFont(D_encoding, mew))
    return;
  if (mew == D_realfont)
    return;
  D_realfont = mew;
#endif
  if (D_xtable && D_xtable[(int)(uint8_t)mew] && D_xtable[(int)(uint8_t)mew][256])
    {
      AddCStr(D_xtable[(int)(uint8_t)mew][256]);
      return;
    }
...
{
  int of, ob;
  static uint8_t sftrans[8] = {0,4,2,6,1,5,3,7};
...
static void
SetBackColor(int mew)
{
    if (!display)
        return;

    SetColor(rend_getfg(&D_rend), mew);
}
#endif /* COLOR */
...
void SetRendition(struct mchar *mc)
{
  ...
  if (nattr2color && D_hascolor && (mc->attr & nattr2color) != 0)
	...
	if (attr2color[i] && (mc->attr & (1 << i)) != 0)
      ...
      mc = &mmc;
      debug2("SetRendition: mapped to %02x %02x\n", (uint8_t)mc->attr, 0x99 - (uint8_t)mc->color);
    }
# ifdef COLORS16
  ...
  if (t - msg >= D_status_buflen)
    {
      char *buf;
      if (D_status_lastmsg)
	buf = (char *)realloc(D_status_lastmsg, t - msg + 1);
      else
	buf = (char *)malloc(t - msg + 1);
      if (buf)
	{
...
void RefreshHStatus()
{
  char *buf;
#ifdef UTF8
  int extrabytes = strlen(hstatusstring) - strlen_onscreen((uint8_t *)hstatusstring, NULL);
#else
  int extrabytes = 0;
#endif
  evdeq(&D_hstatusev);
  ...
  while (from <= to)
      ...
      for (cv = display->d_cvlist; cv; cv = cv->c_next)
	{
	  if (y == cv->c_ye + 1 && from >= cv->c_xs && from <= cv->c_xe)
	    {
#ifdef UTF8
	      int extrabytes = strlen(captionstring) - strlen_onscreen((uint8_t *)captionstring, NULL);
#else
	      int extrabytes = 0;
#endif
  ...
  if (D_obuflen && D_obuf)
    {
      ...
      D_obuflen += GRAIN;
      D_obuffree += GRAIN;
      D_obuf = (char *)realloc(D_obuf, D_obuflen);
    }
  else
    {
      ind  = 0;
