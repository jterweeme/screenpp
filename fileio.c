...
//static char *CatExtra __P((const char *, char *));
...
static char *CatExtra(const char *str1, char *str2)
{
    ...
    if (str2)
    {
        ...
        if ((cp = (char *)realloc(str2, (unsigned) len1 + len2 + add_colon + 1)) == NULL)
            Panic(0, "%s", strnomem);
    ...
    {
        if ((cp = (char *)malloc((unsigned) len1 + add_colon + 1)) == NULL)
            Panic(0, "%s", strnomem);
...
# endif
  ...
  switch (dump) {
    ...
    case DUMP_EXCHANGE:
      ...
      xpublic = !strcmp(fn, DEFAULT_BUFFERFILE);
# ifdef HAVE_LSTAT
      exists = !lstat(fn, &stb);
      if (xpublic && exists && (S_ISLNK(stb.st_mode) || stb.st_nlink > 1)) {
...
FILE *secfopen(const char *name, const char *mode)
{
  FILE *fi;
#ifndef USE_SETEUID
  int flags, fd;
