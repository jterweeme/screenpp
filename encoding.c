...
extern int cjkwidth;
#endif
static int encmatch(const char *, const char *);
#ifdef UTF8
...
static int recode_char_to_encoding(int c, int encoding)
{
    ...
    if ((fp = encodings[encoding].fontlist) != 0)
        while(*fp)
            if ((x = recode_char(c, 0, (uint8_t)*fp++)) != -1)
                return x;
...
static int recode_char_dw_to_encoding(int c, int *c2p, int encoding)
{
    ...
    if ((fp = encodings[encoding].fontlist) != 0)
        while(*fp)
            if ((x = recode_char_dw(c, c2p, 0, (uint8_t)*fp++)) != -1)
                return x;
...
struct mline *recode_mline(struct mline *ml, int w, int from, int to)
{
  ...
  if (w > maxlen)
	...
	{
	  if (rml[i].image == 0)
	    rml[i].image = (uint8_t *)malloc(w);
	  else
	    rml[i].image = (uint8_t *)realloc(rml[i].image, w);
	  if (rml[i].font == 0)
	    rml[i].font = (uint8_t *)malloc(w);
	  else
	    rml[i].font = (uint8_t *)realloc(rml[i].font, w);
	  if (rml[i].fontx == 0)
	    rml[i].fontx = (uint8_t *)malloc(w);
	  else
	    rml[i].fontx = (uint8_t *)realloc(rml[i].fontx, w);
  ...
  if (c >= 0x10000)
    {
      if (c >= 0x200000)
	{
	  AddChar((char)(c & 0x3000000) >> 12 ^ 0xf8);
...
int ToUtf8(char *p, int c)
{
    ...
    if (c >= 0x10000)
    {
        ...
        {
            if (p)
                *p++ = (char)(c & 0x3000000) >> 12 ^ 0xf8;  // verandering hier
...
{
    int c1 = mc->image | (mc->font << 8) | mc->fontx << 16;
    int isdouble = c1 >= 0x1100 && utf8_isdouble(c1);
...
static int encmatch(const char *s1, const char *s2)
{
  int c1, c2;
  do
    {
...
const char *EncodingName(int encoding)
{
...
void ResetEncoding(struct win *p)
{
    ...
    const char *c = encodings[encoding].charsets;
...
int RecodeBuf(uint8_t *fbuf, int flen, int fenc, int tenc, uint8_t *tbuf)
{
  int c, i, j;
...
int LoadFontTranslation(int font, char *file)
{
  ...
  for (;;)
	...
	getc(f); 	/* skip font name (padded to 2 bytes) */
      if ((p = (uint16_t (*)[2])malloc(sizeof(*p) * (i + 1))) == 0)
	break;
      tab = p;
      while(i > 0)
	{
