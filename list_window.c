...
gl_Window_add_group(struct ListData *ldata, struct ListRow *row)
{
  ...
  struct gl_Window_Data *wdata = (struct gl_Window_Data *)ldata->data;
  struct win *group = (struct win *)row->data, *w;
...
static int gl_Window_input(struct ListData *ldata, char **inp, int *len)
{
  ...
  switch (ch)
	...
	{
	  struct win *pw = (struct win *)ldata->selected->prev->data;
	...
	{
	  struct win *nw = (struct win *)ldata->selected->next->data;
      ...
      if (ch >= '0' && ch <= '9')
	{
	  ...
	  for (; row; row = row->next)
	    {
	      struct win *w = (struct win *)row->data;
	      if (w->w_number == ch - '0')
		{
...
display_windows(int onblank, int order, struct win *group)
{
  ...
  wdata = (struct gl_Window_Data *)calloc(1, sizeof(struct gl_Window_Data));
  wdata->group = group;
...
static void WListUpdate(struct win *p, struct ListData *ldata)
{
  ...
  if (!p)
    {
      if (ldata->selected)
	wdata->fore = (struct win *)ldata->selected->data;	/* Try to retain the current selection */
      glist_remove_rows(ldata);
      gl_Window_rebuild(ldata);
      return;
    }
