...
void LPutStr(struct layer *l, const char *s, int n, struct mchar *r, int x, int y)
{
  struct canvas *cv;
  struct viewport *vp;
  const char *s2;
...
int InitOverlayPage(int datasize, struct LayFuncs *lf, int block)
{
  ...
  data = 0;
  if (datasize) {
    if ((data = (char *)calloc(1, datasize)) == 0) {
      free((char *)newlay);
...
void LayPauseUpdateRegion(struct layer *layer, int xs, int xe, int ys, int ye)
{
  ...
  if (layer->l_pause.bottom < ye) {
    ...
    if (layer->l_pause.lines <= ye) {
      ...
      layer->l_pause.left = (int *)realloc(layer->l_pause.left, sizeof(int) * layer->l_pause.lines);
      layer->l_pause.right = (int *)realloc(layer->l_pause.right, sizeof(int) * layer->l_pause.lines);
