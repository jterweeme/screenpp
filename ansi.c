...
static const char *string_t_string[] =
{
  "NONE",
...
static const char *state_t_string[] =
{
  "LIT",			/* Literal input */
...
static void MPutStr (struct win *, const char *, int, struct mchar *, int, int);
...
static void WLogString(struct win *p, const char *buf, int len)
{
  if (!p->w_log)
    return;
...
void SetCharsets(struct win *p, const char *s)
{
    int i;
 ...
 *
 */
void WriteString(struct win *wp, const char *buf, int len)
{
    int c;
#ifdef FONT
    int font;
...
static void Report(const char *fmt, int n1, int n2)
{
  int len;
...
static void MPutStr(struct win *p, const char *s, int n, struct mchar *r, int x, int y)
{
  struct mline *ml;
  int i;
  uint8_t *b;
...
void WMsg(struct win *p, int err, const char *str)
{
