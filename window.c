...
void nwin_compose(struct NewWindow *def, struct NewWindow *mew, struct NewWindow *res)
{
#define COMPOSE(x) res->x = mew->x != nwin_undef.x ? mew->x : def->x
  COMPOSE(StartAt);
  COMPOSE(aka);
  COMPOSE(args);
  COMPOSE(dir);
...
static void zmodem_fin(char *buf, int len, char *data)
{
    ...
    {
      char *s = (char *)"\030\030\030\030\030\030\030\030\030\030";
      int n = strlen(s);
      LayProcess(&s, &n);
    }
}
