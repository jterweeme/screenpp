...
static void  AddCap __P((const char *));
static void  MakeString __P((char *, char *, int, char *));
static char *findcap __P((const char *, char **, int));
static int   copyarg __P((char **, char *));
static int   e_tgetent __P((char *, char *));
static char *e_tgetstr __P((const char *, char **));
static int   e_tgetflag __P((const char *));
static int   e_tgetnum __P((const char *));
#ifdef MAPKEYS
static int   findseq_ge __P((char *, int, unsigned char **));
static void setseqoff(uint8_t *, int, int);
...
int InitTermcap(int wi, int he)
{
  ...
  if (D_CTF)
	...
	{
#ifdef TERMINFO
	  D_CAF = (char *)"\033[3%p1%dm";
	  D_CAB = (char *)"\033[4%p1%dm";
#else
	  D_CAF = (char *)"\033[3%dm";
	  D_CAB = (char *)"\033[4%dm";
#endif
	}
      if (D_OP && InStr(D_OP, (char *)"\033[39;49m"))
        D_CAX = 1;
      if (D_OP && (InStr(D_OP, (char *)"\033[m") || InStr(D_OP, (char *)"\033[0m")))
        D_OP = 0;
      /* ISO2022 */
  ...
  if (D_CG0)
    ...
    {
      D_CS0 = (D_AS && D_AE) ? D_AS : (char *)"";
      D_CE0 = (D_AS && D_AE) ? D_AE : (char *)"";
      D_CC0 = D_AC;
    }
  else
    {
      D_CS0 = D_CE0 = (char *)"";
      D_CC0 = 0;
      D_AC = (char *)"";    /* enable default string */
    }
...
static int findseq_ge(char *seq, int k, uint8_t **sp)
{
  uint8_t *p;
  int j, l;
...
static void setseqoff(uint8_t *p, int i, int o)
{
  uint8_t *q;
  int l, k;
...
static int addmapseq(char *seq, int k, int nr)
{
  ...
  if (D_nseqs + 2 * k + 4 >= D_aseqs)
    {
      D_kmaps = (uint8_t *)xrealloc((char *)D_kmaps, D_aseqs + 256);
      D_aseqs += 256;
      p = D_kmaps + i;
    }
  D_seqp = D_kmaps + 3;
 ...
 */
static void AddCap(const char *s)
{
  int n;
...
char *MakeTermcap(int aflag)
{
  ...
  for (i = T_CAPS; i < T_ECAPS; i++)
	  ...
	  if (act->nr != RC_ILLEGAL)
	    {
	      if (act->nr == RC_STUFF)
		{
		  MakeString((char *)term[i].tcname, buf, sizeof(buf), act->args[0]);
		  AddCap(buf);
		}
	      continue;
	    }
	...
	case T_STR:
	  if (D_tcs[i].str == 0)
	    break;
	  MakeString((char *)term[i].tcname, buf, sizeof(buf), D_tcs[i].str);
	  AddCap(buf);
	  break;
	case T_FLG:
	  if (D_tcs[i].flg == 0)
...
static char *findcap(const char *cap, char **tepp, int n)
{
  char *tep;
  char c, *p, *cp;
...
static char *e_tgetstr(const char *cap, char **tepp)
{
  char *tep;
  if ((tep = findcap(cap, tepp, 0)))
    return (*tep == '@') ? 0 : tep;
  return tgetstr((char *)cap, tepp);
}

static int e_tgetflag(const char *cap)
{
  char buf[2], *bufp;
  char *tep;
  bufp = buf;
  if ((tep = findcap(cap, &bufp, 2)))
    return (*tep == '@') ? 0 : 1;
  return tgetflag((char *)cap) > 0;
}

static int e_tgetnum(const char *cap)
{
  char buf[20], *bufp;
  char *tep, c;
  int res, base = 10;
