...
int FindSocket(int *fdp, int *nfoundp, int *notherp, char *match, bool *is_sock)
{
    DIR *dirp;
    struct dirent *dp;
    struct stat st;
  ...
  if (ndead && !quietflag)
    {
      const char *m = "Remove dead screens with 'screen -wipe'.";
      if (wipeflag)
...
int
MakeClientSocket(int err, bool socket)
{
  if (socket)
...
# endif				/* POW_DETACH */
  ...
  /* create new display */
  GetTTY(i, &Mode);
  if (MakeDisplay(user, m->m_tty, attach ? m->m.attach.envterm : (char *)"", i, pid, &Mode) == 0)
    {
...
ReceiveMsg()
{
  ...
  if (!is_socket)
  ...
  else
    {
      ...
      if ((ns = accept(ns, (struct sockaddr *)&a, (unsigned int *)&len)) < 0)
        {
          Msg(errno, "accept");
          return;
        }
...
ReceiveRaw(int s)
{
  ...
  if (!is_socket)
    ...
    {
      len = sizeof(a);
      s = accept(s, (struct sockaddr *)&a, (unsigned int *)&len);
      if (s < 0)
        {
          Msg(errno, "accept");
          return;
