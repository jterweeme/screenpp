...
static void PadStr(const char *, int, int, int);

extern char *wliststr;
extern char *wlisttit;

void exit_with_usage(const char *myname, const char *message, char *arg)
{
...
void display_help(char *klass, struct action *ktabp) {
  ...
  for (; crow < flayer->l_height - 3; crow++) {
    if (helpdata->grow < 1) {
      if (ktabp == ktab)
        ...
        sprintf(cbuf,"Command class: '%.80s'", helpdata->klass);
...
void display_bindkey(const char *title, struct action *tab)
{
    ...
    for (int i = 0; i < KMAP_KEYS+KMAP_AKEYS+kmap_extn; i++)
    {
  ...
  y = 2;
    ...
    if (i < KMAP_KEYS) {
      ...
      if (act->nr == RC_ILLEGAL)
        continue;
      del = *p++ = ':';
      s = (char *)term[i + (T_CAPS - T_OCAPS + T_CURSOR)].tcname;
      sl = s ? strlen(s) : 0;
      xch = (char *)"[A]";
    }
...
static void PadStr(const char *str, int n, int x, int y) {
  int l;
