...
int join_with_cr =  0;
int compacthist = 0;
uint8_t mark_key_tab[256]; /* this array must be initialised first! */
...
int GetHistory()	/* return value 1 if copybuffer changed */
{
...
static void MarkRedisplayLine(int y, int xs, int xe, int isblank)
{
    ...
    struct mchar mchar_marked = mchar_so;
    int wy = D2W(y);
    ml = WIN(wy);
...
static void MarkProcess(char **inbufp, int *inlenp)
{
    ...
    markdata = (struct markdata *)flayer->l_data;
    fore = markdata->md_window;
    struct acluser *md_user = markdata->md_user;
	  ...
	  if (!markdata->second)
	      ...
	      if (newcopylen > 0)
		{
		  ...
		  if (md_user->u_plop.buf)
		    md_user->u_plop.buf = (char *)realloc(md_user->u_plop.buf,
			(unsigned) (md_user->u_plop.len + newcopylen + 3));
		  else
		    {
		      md_user->u_plop.len = 0;
		      md_user->u_plop.buf = (char *)malloc((unsigned) (newcopylen + 3));
		    }
		  if (!md_user->u_plop.buf)
		    {
		      MarkAbort();
